...
2026-09-12 | G1 | a70dd8c | **Discover: barra de búsqueda + pills de tipo con más peso visual.** Primera pieza del Bloque G (cerrar la brecha visual real entre la app y F0), en paralelo a G2 (worktree aislado, `NavLinks.tsx`). Diagnóstico previo (abierto en BLOQUE G): el grid bento (F3b) y el match score real (F3a) funcionan, pero el cromado interactivo alrededor de las cards en Discover (`DiscoverClient.tsx`) nunca se tocó durante F1-F7. Tres cambios: (a) `<SearchBar>` (componente ya existente, `/api/search` + debounce + navegación a `/search?q=`) integrado bajo la cabecera de Discover con placeholder propio (`discover.searchPlaceholder`, +1 clave i18n es/en) — cero lógica de búsqueda duplicada (regla técnica 2/3), mismo componente que ya usa `SearchClient`; (b) `TYPE_EMOJI` (mapa `DiscoverType→string`: ✨🎬📺⛩️📚🈷️🎮💬) delante de cada label de pill de tipo en un `<span aria-hidden="true">` — puramente decorativo, verificado con test nuevo que el nombre accesible del radio (`getByRole('radio', {name: 'movie'})`) sigue siendo solo la etiqueta i18n; acento activo intacto (`accent-positive`, sin reabrir la decisión de paleta única de F2); (c) `h1` de Discover (`text-4xl tracking-wide`) → `text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight` (misma escala que el `h1` de `MediaDetail.tsx`, F4) + `IconCompass` del sistema propio F1b junto al título (nunca un emoji genérico para el icono de cabecera). +2 tests de regresión en `DiscoverClient.test.tsx`. Verificación visual real: harness temporal no commiteado (mismo patrón que F4/F3b) montando `DiscoverClient` con fixture de 8 items y `fetch('/api/discover')` interceptado en el navegador, capturado con Chromium headless (`/opt/pw-browsers/chromium-1194`) en mobile (390px) y desktop (1440px) contra el CSS de Tailwind realmente compilado por `next dev` — confirma header/búsqueda/pills/grid bento/filtros/paginación sin regresión de layout, borrado antes de este commit. tsc 0, lint 0, vitest **1313 passed**.

2026-09-12 | G2 | ed89e45 | **Navegación desktop (`NavLinks`) con el mismo peso visual que la navegación móvil.** Trabajo en paralelo a G1 (agente independiente en worktree aislado, mismo bloque G — cerrar la brecha visual real entre la app y F0). `NavLinks.tsx`: de enlaces de solo texto + punto de estado activo a icono+etiqueta (`IconHome`/`IconCompass`/`IconLibrary`/`IconLists`/`IconChat`/`IconFriends`/`IconGroups`/`IconIdea`, los 8 sugeridos, ninguno cambiado) con pill activa (`bg-accent-positive`/`text-on-accent-positive rounded-full`, mismo lenguaje que las pills de tipo de Discover); inactivo en `text-text-secondary` (token ya usado en `MediaDetail.tsx` post-F5, no el alias legacy `text-muted`). Badge de no-leídos (E96) sin cambios de lógica/aria-label, reposicionado sobre el icono de chat con `ring-2 ring-surface-default` para no fundirse con la pill activa. Verificación visual real con harness temporal (no commiteado): header real montado vía un rewrite temporal de `next.config.mjs` (revertido) para que `usePathname()` apuntase de verdad a `/home`/`/library` — no simulado, mismo mecanismo de detección de ruta activa que producción. Chromium headless confirma la pill cambiando de item entre las dos rutas, badge "7" y "99+" sobre el icono de chat, y la nav correctamente oculta en mobile (390px, `hidden md:flex`) sin romper el header. Test actualizado con cobertura de icono por link y clases de pill activa/inactiva, sin perder los casos previos. tsc 0, lint 0, vitest **1313 passed**.
