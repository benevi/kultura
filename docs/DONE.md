...
2026-09-12 | E-GAMES-STEAM | (este commit) | **Ficha de juego enriquecida con Steam Store API (RAWG intacto para Descubrir).** Alcance según la decisión ya tomada: Steam NO tiene endpoint público de búsqueda/catálogo paginado (solo detalle por `appid`), así que Descubrir, los listados y la paginación siguen siendo de RAWG y Steam entra solo en `/media/game/{id}`. Nuevo `src/lib/api/steam.ts`. **Resolución del `appid`, en dos pasos:** (1) `stores[]` del DETALLE de RAWG — se parsea `/app/{id}` de la URL curada, reconociendo Steam por `store.slug`, `store.domain` o `store.id===1`, y con un segundo barrido que acepta la URL de Steam aunque falten los metadatos de tienda; (2) si RAWG no lo trae, best-effort por nombre con `store.steampowered.com/api/storesearch/`, que **solo** acepta coincidencia exacta normalizada (sin acentos, signos ni mayúsculas): un "parecido" enseñaría el precio de otro juego, peor que no enseñar nada (test de regresión: "Hades" NO acepta "Hades II"). Luego `appdetails?appids=…&l=spanish|english&cc=ES|US` con idioma y país derivados del locale, y descarte de DLC/demos (`type !== "game"`: un DLC tiene ficha y precio propios que no representan al juego). Nuevo `SteamSection` (Server Component, mobile-first): precio con precio original tachado y −% cuando hay descuento, Metacritic, idiomas soportados —parseados del HTML que devuelve Steam en `supported_languages`, quitando etiquetas, la nota de doblaje y los asteriscos—, hasta 4 capturas enlazando a la versión grande, y enlace a la tienda con `rel="noopener"`. Cada sub-bloque se pinta solo si el dato existe (nada de huecos vacíos). `RawgGame.stores` añadido al tipo (solo lo trae el detalle); `next.config.mjs` += `*.steamstatic.com` y `shared.akamaihd.net` en `remotePatterns`; 6 claves i18n nuevas en `media_detail` es/en (paridad **602=602**). **Contrato de degradación (el punto importante):** `getSteamInfoForGame` no lanza NUNCA — juego que no está en Steam, appid no resoluble con confianza, red caída, `success:false` o JSON con shape inesperado → `null`, y la ficha se renderiza igual sin la sección; hay un test por cada uno de esos casos. +35 tests (27 del cliente + 8 del componente). tsc 0, lint 0, vitest **1359 passed**. **Limitación de verificación:** no se pudo abrir la ficha de juego en este entorno (`getGame` de RAWG exige `RAWG_API_KEY` real — sin ella la ruta hace `notFound()` — y `store.steampowered.com` está bloqueado por la política de egress del agente), así que el shape de Steam se implementó defensivo (todos los campos opcionales) contra la documentación pública y queda **pendiente la validación visual** del bloque en dev/preview con keys reales (anotado en BACKLOG).

---

2026-09-12 | E79-s3 | (este commit) | **Paginación coherente entre categorías: tope común + profundidad real del agregado.** Origen: el usuario reportó que la categoría "todos" ofrecía ~5 páginas mientras "películas" ofrecía ~500. Diagnóstico: dos causas independientes. **(1) Cada familia tenía su propio techo** — TMDB 500 (E89), book 50 (heredado de Open Library), comic y game NINGUNO (se exponía el conteo crudo del proveedor: RAWG `count=900360` → 45018 páginas fantasma), Jikan su `last_visible_page`. Fix: nuevo `src/lib/api/pagination.ts` con `PAGE_SIZE` y `DISCOVER_MAX_PAGES = 100` (módulo sin dependencias a propósito: `discover.ts` y `aggregate.ts` ya se importan mutuamente y una constante compartida en cualquiera de los dos añadía otra arista al ciclo — el intento previo rompió los tests con un mock parcial). Las 7 familias capan `totalPages` a ese valor y `hasMore` se gobierna contra él; el guard "página fuera de rango → vacío SIN banner de error y sin llamar al proveedor" pasa de existir solo para movie/tv a ser **uno común al principio de `fetchDiscoverData`** (antes anime/manga/book/comic/game llamaban al proveedor con una página imposible). Elección de 100 razonada en el módulo: 2000 ítems por familia, por debajo del tope servible de todos los proveedores (así la "última página" de la UI numerada nunca da 4xx) y alcanzable por el agregado. **(2) El agregado `all` pedía SIEMPRE `page=1`** a las 7 familias → pool máximo 7×20=140 ítems → de ahí las ~5-7 páginas. Fix: la página global se traduce a una **RONDA** — `roundForPage(P)` es la página nativa que se pide a cada familia y `offsetInRound(P)` la porción del pool combinado que se sirve (`PAGES_PER_ROUND = 4`), más un guard propio del agregado sobre la página GLOBAL (el guard por familia mira la página nativa de la ronda, que es mucho más baja, así que sin él el agregado servía contenido más allá del tope que la UI anuncia). `totalPages` viaja como `null` mientras alguna familia reporte `hasMore` (ventana abierta, patrón E79-s2) y como número exacto —páginas de rondas anteriores + las que llena ésta, acotado a la ventana de la ronda— cuando el pool se agota. **`PAGES_PER_ROUND=4` y no 7 es deliberado:** el pool real casi nunca llega a 140 (una familia puede devolver menos de 20, fallar, o quedar recortada por sus post-filtros), y con 7 páginas por ronda las últimas 1-2 de CADA ronda saldrían cortas o vacías con "siguiente" activo — peor UX que la incoherencia que se arregla; con 4 (80 ítems de un pool típico de 100-140) toda página va llena, a cambio de descartar la cola de la ronda (el agregado anterior ya descartaba 120 de 140). +16 tests (tope por familia incluido comic/game, guard común a las 7, rondas/offsets, totalPages null vs exacto, tope en el agregado). tsc 0, lint 0, vitest **1324 passed**. Remanente explícito → **E79-s4** en BACKLOG: caché de pool (evitar 4×7 peticiones idénticas por ronda y no descartar la cola) + back-fill de páginas cortas por post-filtro.

---

2026-09-12 | E6 + E98 | (este commit) | **E6 ya estaba hecha: verificada y docs corregidos (E98).** E6 ("ComicVine implementado") figuraba ABIERTA en BACKLOG y CLAUDE.md afirmaba "clave presente, sin handler", pero el repo dice otra cosa: `src/lib/api/comicvine.ts` + `comicvine-maps.ts` están enchufados end-to-end — Descubrir (`discover.ts` case `"comic"` → `getRecentComics` con filtros sort/año/editorial y la lista negra de editoriales de manga), búsqueda (`search.ts` → `searchComics`), ficha (`media/[type]/[id]` → `getComic`) y `normalizeComic`; key server-only vía `env.COMICVINE_KEY`. Evidencia ejecutada: **35 tests unit en verde** (comicvine 20 + comicvine-maps 15). NO existe `/api/comics/route.ts` y no hace falta: el cómic se sirve por `/api/discover?type=comic` como el resto de familias — la redacción de E6 describía un diseño que nunca se usó. **Limitación de verificación:** no se pudo hacer una llamada en vivo (sin `COMICVINE_KEY` en el entorno y host bloqueado por la política de egress del agente); la evidencia es tests con mocks + lectura de código. Cómics no necesita trabajo de idioma: ComicVine es solo inglés, limitación aceptada y ya documentada junto a la de Jikan. **E98 cerrada de paso** (las 3 afirmaciones de CLAUDE.md sincronizadas con el repo): nota de ComicVine corregida en la tabla de APIs y en Env vars; 17→**18 tablas** (`group_invitations`, migración 20260601000002) y 49→**53 policies**, aclarando que el bloque SQL es la baseline B2; 594→**596 keys** de i18n (recontadas: es 596 = en 596). Residuo: `ESTADO_PROYECTO.md:387` sigue diciendo `claude-sonnet-4-6` → nueva deuda **E-DOCS-ESTADO** en BACKLOG.
