...
- [x] **E6. (CERRADA 2026-09-12 — YA ESTABA HECHA, solo faltaba verificarlo y corregir los docs)** ComicVine implementado.
  Verificación de estado real (DEBUG_PRINCIPLES #1): `src/lib/api/comicvine.ts` (+ `comicvine-maps.ts`) existe y está enchufado end-to-end — Descubrir (`discover.ts` case `"comic"` → `getRecentComics`, con filtros sort/año/editorial), búsqueda (`search.ts` case `"comic"` → `searchComics`), ficha (`media/[type]/[id]` → `getComic`) y `normalizeComic` en el normalizer; 35 tests unit en verde (`comicvine.test.ts` 20 + `comicvine-maps.test.ts` 15). La key se lee server-only (`env.COMICVINE_KEY`, lanza si falta → la familia cómic queda vacía sin afectar al resto). NO existe ni hace falta `/api/comics/route.ts`: el catálogo de cómic se sirve por `/api/discover?type=comic`, como el resto de familias.
  Cómics NO necesita trabajo de idioma: ComicVine es solo inglés y no hay alternativa gratuita con catálogo ES → **limitación aceptada**, documentada en `src/lib/api/locale.ts` y en la tabla de APIs de CLAUDE.md (igual que Jikan).
  Pendiente de verificación real: una llamada en vivo a comicvine.gamespot.com (no posible en la sesión 2026-09-12: sin `COMICVINE_KEY` en el entorno y con el host bloqueado por la política de egress del agente). Evidencia usada: tests unit con mocks + lectura del código.
...
- [ ] **E79. Paginación: mejora (UX)** — _slice 1 hecho, slice 2 pendiente_
  ...
  - [x] **slice 2 — totalPages `null` con post-filtro activo** (CERRADA 2026-07-16, commit `a208db0`): cuando una familia tiene post-filtro server-side ACTIVO (tv+temporadas, manga+volumenes, game+valoracion/estado/modojuego/duracionmedia) el `totalPages` crudo miente (RAWG count=900360 → 45018 páginas fantasma). `DiscoverResult.totalPages` → `number | null`; `fetchDiscoverData` devuelve `null` vía `hasActivePostFilter` (NSFW global excluido: recorte marginal). `Pagination` con `total===null` pinta ventana abierta `[1] … [c-1][c][c+1] …` sin última `[N]` ni salto a ella; "siguiente" sigue gobernado por `hasMore`. `DiscoverClient` distingue `null` (significativo) de `undefined` (default 1). +88 tests discover +59 pagination. Validado en prod (regla #11). El overfetch+pool / saltos arbitrarios se extrajo a **E79-s3** (cerrada 2026-09-12; su remanente de caché/cursor es **E79-s4**). tsc 0, lint 0.
  - [ ] **slice 2 (remanente → E79-s3) — cachear pool + profundizar + saltos arbitrarios**: cachear el pool del agregado `all` (hoy re-fetcha page 1 de las 7 familias en cada página) y el overfetch de `comic` (pide 100, tira ~80); profundizar la paginación real más allá del pool ya traído (back-fill / cursor por familia para no truncar la profundidad navegable). **NUEVO (por slice 1b):** la UI numerada permite SALTOS arbitrarios (clic directo a página N lejana) → cubrir el caso "salto a página muy filtrada cae corta/vacía": con post-filtros agresivos, una página intermedia/alta puede devolver pocos o cero items aunque `hasMore` siga true → back-fill/cursor debe garantizar páginas llenas (o señalar el final real) en saltos no contiguos.
...
- [x] **E93. (CERRADA 2026-07-16 — commit `49f3dd2`) DMs solo entre amigos.** Hallazgo S6 (auditoría 2026-07-07): `create_conversation_with_members` no exigía amistad. **Decisión de producto: restringir a amigos.** Migración `20260716000001_dm_only_between_friends.sql` — la RPC exige `friendships.status='accepted'`; `POST /api/chat` → 403 `not_friends` si no hay amistad. `ChatClient` maneja el 403 con i18n (+1 clave es/en), +29 líneas tests. Validado en prod por el usuario (regla #11). Detalle en DONE.md.
...
- [x] **E96. (CERRADO 2026-07-16, commit 7553ef1)** Chat realtime — mensajes entrantes sin recargar (Supabase Realtime) + badge/notificación de no-leídos. **Nota de cierre:** s1 (mensajes en vivo en conversación abierta) ya existía (A5.8); s2 entrega el badge global de **conversaciones** no leídas (diseño confirmado por el usuario): `GET /api/chat?countOnly=1` (una query con join + count anidado), `UnreadChatProvider` con suscripción realtime global e `markConversationRead`, badges en BottomNav/NavLinks. Incluye fix de bug latente: supabase-js no propaga el JWT al socket realtime en `INITIAL_SESSION` (sesión de cookies) → canal unido como anon queda SUBSCRIBED pero sordo (WALRUS); afectaba también al realtime por-conversación tras reload. Fix: suscripción gateada por token + `await setAuth` antes de subscribe + `onAuthStateChange` como fuente. Detalle en DONE.md.
...
- [ ] **E97. Unificar vars legacy `NEXT_PUBLIC_TMDB_API_KEY` / `NEXT_PUBLIC_RAWG_KEY` con los nombres canónicos.** Solo las usan los tests de contrato (`tests/contract/tmdb.contract.test.ts`, `tests/contract/rawg.contract.test.ts`, doc en `vitest.contract.config.ts`); el resto del código usa `TMDB_API_KEY` / `RAWG_API_KEY` (post-A4). Renombrar en los tests + `.env.local` + `.env.example`. Deuda menor, sin prioridad.
  Hecho cuando: 0 ocurrencias de `NEXT_PUBLIC_TMDB_API_KEY` / `NEXT_PUBLIC_RAWG_KEY` en el repo y los tests de contrato pasan con los nombres canónicos.

- [x] **E79-s3. (CERRADA 2026-09-12) Paginación coherente entre categorías: tope común + profundidad real del agregado**
  Origen: el usuario reportó que "todos" ofrecía ~5 páginas mientras "películas" ofrecía ~500. Dos causas, las dos arregladas:
  1. **Tope COMÚN** (`src/lib/api/pagination.ts` → `DISCOVER_MAX_PAGES = 100`): antes cada familia tenía su propio techo (TMDB 500, book 50, comic/game NINGUNO → se exponía el conteo crudo: RAWG count=900360 = 45018 páginas fantasma). Ahora las 7 familias y el agregado capan `totalPages` al mismo valor y `hasMore` se gobierna contra él. El guard de "página fuera de rango → vacío sin banner de error, sin llamar al proveedor" pasa de ser solo de movie/tv (E89) a ser **común a todas las familias** y también al agregado (que lo necesita aparte, porque el guard por familia mira la página NATIVA de la ronda, no la global). Elección de 100 razonada en el módulo: 2000 ítems por familia, por debajo del tope servible de todos los proveedores y alcanzable por el agregado.
  2. **Profundidad real del agregado** (`aggregate.ts`): antes pedía SIEMPRE `page=1` a las 7 familias (pool máx 140 ítems → de ahí las ~5-7 páginas). Ahora la página global se traduce a una **ronda**: `roundForPage(P)` es la página nativa que se pide a cada familia y `offsetInRound(P)` la porción del pool combinado que se sirve, con `PAGES_PER_ROUND = 4`. `totalPages` es `null` mientras alguna familia tenga más páginas (ventana abierta, patrón E79-s2) y un número exacto cuando el pool se agota.
  `PAGES_PER_ROUND=4` (no 7) es deliberado: el pool real casi nunca llega a 140 (familias que devuelven menos de 20, que fallan, o recortadas por post-filtros), y con 7 las últimas páginas de cada ronda saldrían cortas/vacías con "siguiente" activo. +16 tests (tope por familia, guard común, rondas, totalPages null/exacto). tsc 0, lint 0, vitest 1324 passed.
  **Lo que NO cubre → E79-s4** (abajo): sigue sin caché de pool ni cursor, así que cada página global re-hace las 7 peticiones de su ronda y se descarta la cola de cada ronda; y el caso original de "página corta por post-filtro agresivo dentro de una familia" sigue sin back-fill.

- [ ] **E79-s4. Caché de pool + cursor (resto de E79-s3)**
  Extraído al cerrar E79-s3 (2026-09-12). Tres cosas quedan pendientes:
  1. **Cola de ronda descartada:** el agregado sirve 4 páginas (80 ítems) de un pool de hasta 140 → se tiran entre 20 y 60 ítems por ronda. Con un pool cacheado (KV/memoria por `(filtros, sort, ronda)`) se servirían las 7 páginas sin descartar nada.
  2. **Re-fetch redundante:** cada página global dispara las 7 peticiones de su ronda otra vez (4 páginas de la misma ronda = 4×7 peticiones idénticas). Cachear el pool lo reduce a 1×7 por ronda.
  3. **Back-fill por post-filtro (caso original de E79-s3):** con post-filtros agresivos (temporadas, volumenes, suite de game, año×book) una página intermedia de una familia puede salir corta aunque `hasMore` siga true; falta rellenar hasta `PAGE_SIZE` pidiendo la página siguiente del proveedor, o señalar el final real. Aplica también a saltos no contiguos de la paginación numerada.
  Hecho cuando: páginas llenas (o final real señalado) también con post-filtros activos y en saltos arbitrarios, sin re-fetch redundante del pool.

- [x] **E100. `components/ui/button.tsx`: foco de teclado usa tokens shadcn muertos (`ring-ring`, `ring-offset-background`)** — _cerrada 2026-09-12_
  Hallazgo de F5 (auditoría de accesibilidad post-rediseño): `focus-visible:ring-ring focus-visible:ring-offset-2` en `src/components/ui/button.tsx` no generaba ninguna regla CSS real — `--ring`/`--background` no están registrados como colores en `tailwind.config.ts`. No era un fallo de accesibilidad (Tailwind aplicaba su azul por defecto cuando el color no resolvía, el foco seguía siendo visible) pero sí inconsistencia de marca — ya señalado como candidato en el cierre de F2, nunca registrado hasta F5. Fix: `ring-ring`/`ring-offset-background` → `ring-accent-positive`/`ring-offset-surface-base` (mismo patrón que `KButton`). Confirmado en el CSS realmente compilado (`next build`): `.focus-visible\:ring-accent-positive:focus-visible{--tw-ring-color:var(--accent-positive)}` ahora sí se genera. Alcance deliberadamente contenido al anillo de foco — las variantes `default`/`destructive`/`outline`/`link` de este mismo componente siguen con otros tokens shadcn muertos (`bg-primary`, `bg-destructive`, etc.), sin consumidores reales que las usen hoy (`RecommendButton`/`RecommendModal` solo usan `ghost`/`primary`) — no registrado como tarea nueva por no tener impacto visible mientras no se usen.
  Hecho cuando: el foco de teclado en esos botones usa el verde de marca (`accent-positive`, mismo patrón que `KButton`) en vez del azul por defecto de Tailwind, verificado en el CSS compilado real (no solo leyendo la clase). tsc 0, lint 0, vitest **1311 passed**, build OK.
...
- [ ] **E-MANGA-SOURCE. La familia `manga` NO usa MangaDex: la sirve Jikan (hallazgo 2026-09-12).**
  Hallazgo al cerrar E-MANGADEX-LOCALE, contradice la asunción de la tabla de APIs ("MangaDex ya en uso" para manga): `discover.ts` case `"manga"` llama a `getPopularManga`/`discoverManga` de **Jikan**, `search.ts` case `"manga"` llama a `searchManga` de **Jikan**, y la ficha `media/[type]/[id]` resuelve manga con `getManga` de **Jikan**. De `mangadex.ts` solo se consumen el tipo `MangaDexManga` y `extractMangaCover` (desde `normalizer.ts`), y `normalizeMangaDex` solo lo ejercitan los tests. Consecuencia real: el catálogo de manga sigue en inglés/japonés (limitación de Jikan) aunque el cliente de MangaDex ya esté localizado.
  Alcance de la migración (pendiente): (1) mapear los triggers de `TYPE_FILTERS.manga` (genero/anio/valoracion/demografia/estado/volumenes/idioma/sort) a params de MangaDex (`includedTags[]`, `publicationDemographic[]`, `status[]`, `year`, `order[…]`, `contentRating[]`) — es un mapa nuevo tipo `mangadex-maps.ts`; (2) paginación `offset`/`limit` + `total` (MangaDex no usa `page`); (3) **compatibilidad de ids**: hoy `manga_{malId}` (numérico, Jikan) vs `manga_{uuid}` (MangaDex) → la ficha debe enrutar por forma del id (numérico → Jikan, UUID → MangaDex) para no romper las bibliotecas ya guardadas, mismo patrón que el fallback legacy de libros en E-BOOKS-GOOGLE; (4) `contentRating[]=safe,suggestive` para no regresar el filtro NSFW (E86).
  **Bloqueo de verificación:** no se migró en la sesión 2026-09-12 porque el entorno del agente no tiene salida a `api.mangadex.org` (política de egress) → imposible validar shapes, filtros ni paginación contra la API real, y hacerlo a ciegas arriesgaba el matriz de filtros de manga ya en verde. Requiere una sesión con red.
  Hecho cuando: Descubrir/búsqueda/ficha de manga se sirven con MangaDex, con los filtros actuales sin regresión, paginación coherente, ids legacy de Jikan aún resolubles y catálogo en el idioma activo.

- [x] **E-GAMES-STEAM. (CERRADA 2026-09-12) Ficha de juego enriquecida con Steam Store API.** RAWG se queda íntegro para Descubrir/listados/paginación (decisión ya tomada: Steam no tiene endpoint público de búsqueda/catálogo); Steam entra SOLO en `/media/game/{id}`. Nuevo `src/lib/api/steam.ts`: resolución del `appid` por la vía fiable (`stores[]` del detalle de RAWG, parseando `/app/{id}` de la URL curada) con **best-effort por nombre** (`storesearch`) de reserva, que solo acepta coincidencia EXACTA normalizada (sin acentos/signos/mayúsculas) — un "parecido" mostraría el precio de otro juego. `appdetails` con `l=spanish|english` + `cc=ES|US` derivados del locale; DLC/demos (`type != "game"`) se descartan. Nuevo `SteamSection` (precio + descuento, Metacritic, idiomas soportados parseados del HTML de Steam, hasta 4 capturas, enlace a la tienda), mobile-first y solo visible si hay datos. `RawgGame.stores` añadido al tipo; `next.config.mjs` += `*.steamstatic.com` y `shared.akamaihd.net`; 6 claves i18n nuevas es/en (paridad 602=602). **Contrato de degradación:** `getSteamInfoForGame` NUNCA lanza — juego que no está en Steam, appid no resoluble, red caída, `success:false` o shape inesperado → `null` y la ficha se pinta igual sin la sección. +35 tests (27 cliente + 8 componente). tsc 0, lint 0, vitest 1359 passed.
  **Pendiente de validación visual** (como E94): la ficha de juego no se puede renderizar en el entorno del agente — `getGame` de RAWG necesita `RAWG_API_KEY` real (sin ella la ruta hace `notFound()`) y `store.steampowered.com` está bloqueado por la política de egress. Falta abrir `/media/game/{id}` en dev o preview con las keys reales y comprobar el bloque en móvil y desktop.

- [x] **E-TMDB-LOCALE. (CERRADA 2026-09-12) Idioma de catálogo atado al locale activo (TMDB).** `tmdb.ts:114` fijaba `language=es-ES` en toda petición → títulos/sinopsis siempre en español con la app en inglés. Nuevo `src/lib/api/locale.ts` (mapeo locale→proveedor, fuente única) + `locale` opcional en todas las funciones de `tmdb.ts`; resuelto en el borde con `getLocale()` (`/api/discover`, `/api/search`, `/api/genre-news`) y desde `params` en la ficha `/[locale]/media/[type]/[id]`; propagado por discover/aggregate/search/genre-news/recomendaciones IA. Incluye fix de `watch/providers` (región derivada del locale en vez de `ES` fijo, extractor region-aware). +26 tests. tsc 0, lint 0, vitest 1273 passed.

- [x] **E98. (CERRADA 2026-09-12) Sincronizar CLAUDE.md con codigo real.** Las 3 afirmaciones corregidas: (1) la nota de ComicVine "clave presente, sin handler — E6" era FALSA → la tabla de APIs dice ahora "implementado y en uso" con sus archivos, y la línea de `COMICVINE_KEY` en Env vars explica el efecto real de su ausencia; (2) "17 tablas / 49 policies" → **18 tablas** (la 18ª es `group_invitations`, migración `20260601000002`) y **53 policies** vigentes tras las 14 migraciones, dejando claro que el bloque SQL de abajo es la baseline B2 y no incluye `group_invitations`; (3) "594 keys c/u" → **596** (recontado sobre `messages/es.json` y `messages/en.json`: 596 = 596, paridad OK). ESTADO_PROYECTO.md:387 (`claude-sonnet-4-6`) sigue siendo la fuente desactualizada del modelo; el código y CLAUDE.md coinciden en `claude-haiku-4-5` → anotado como deuda menor de documentación en E-DOCS-ESTADO.
...
- [x] **F5. Auditoría de accesibilidad post-rediseño** — _cerrada 2026-09-12_
  Auditoría real (cálculo de contraste WCAG, no "a ojo") sobre F1-F4:
  - **Contraste — 4 fallos encontrados y corregidos** en `MediaDetail.tsx` (introducidos por F4): `text-tertiary` sobre `surface-base`/`surface-elevated` da 3.97:1 / 3.18:1 en texto normal pequeño (originalTitle, fuente del rating en el hero, label y hint de los tiles de "Detalles") — por debajo del mínimo AA de 4.5:1 para texto normal. Subidos a `text-secondary` (5.94:1 sobre `surface-elevated`, 7.42:1 sobre `surface-base`, ambos ≥AA). El único uso de `text-tertiary` que se deja igual es el placeholder de póster sin imagen (texto grande en negrita, ≥18.66px bold → umbral AA de solo 3:1, cumple con 3.18:1).
  - **Orden DOM/tabulación del grid bento (F3b) — verificado matemáticamente, sin bug real:** el patrón de 6 celdas (`BENTO_CELL_CLASSES`) tila exactamente 12 columnas × 2 filas (5+4+3, todas `row-span-2`) seguido de 12 columnas × 1 fila (4+4+4) en cada ciclo completo, en ambos breakpoints (mobile 2 columnas, desktop 12) — cero huecos dentro de un ciclo ni al final de una lista parcial (no hay item posterior que "adelantarse" a rellenar un hueco). `grid-flow-row-dense` es por tanto inerte con este patrón concreto (no hay huecos que compactar) — el orden visual coincide con el orden DOM/tabulación. No requiere cambio de código.
  - **`prefers-reduced-motion` — verificado, cobertura global confirmada** (regla `@media (prefers-reduced-motion: reduce)` en `globals.css`, selector universal `*,*::before,*::after`): cubre automáticamente cualquier transición/animación introducida por F1-F4 (hover scale de `MediaCard`, etc.) sin necesidad de tocarla caso a caso.
  - **Foco de teclado — verificado en el CSS realmente compilado:** los botones basados en `src/components/ui/button.tsx` (usado por `RecommendButton`/`AddToListButton`, tocados por F4 en la fila de acciones sociales) usan `focus-visible:ring-ring`/`ring-offset-background`, tokens shadcn nunca registrados en `tailwind.config.ts` (ya señalado como candidato en el cierre de F2) — confirmado por grep en el CSS compilado que esas clases no generan ninguna regla. El foco SIGUE siendo visible (Tailwind aplica su azul por defecto al no poder resolver el color de marca), así que no es un fallo de accesibilidad, pero es una inconsistencia de marca. Registrado como **E100** en vez de arreglado aquí — toca un componente compartido (`button.tsx`) usado fuera del alcance de Bloque F.
...
- [ ] **E-DOCS-ESTADO. `ESTADO_PROYECTO.md` desactualizado (deuda menor de docs).** Detectado al cerrar E98: `ESTADO_PROYECTO.md:387` dice `claude-sonnet-4-6` cuando el modelo real es `claude-haiku-4-5` (`src/lib/claude/recommendations.ts`, coincide con CLAUDE.md). El archivo (90 KB) no se mantiene al día y arrastra más afirmaciones de fases antiguas; decidir si se poda a un histórico marcado como tal o se retira.
  Hecho cuando: el modelo corregido y el archivo marcado explícitamente como histórico (o retirado), sin contradicciones vivas con CLAUDE.md.
