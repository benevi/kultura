...
## BLOQUE G — Cerrar la brecha visual real entre la app y F0 (cromado interactivo)

> Abierto 2026-09-12 tras verificación con capturas reales de producción por el usuario ("no se parece en nada la página real al diseño"). Diagnóstico verificado contra el código real (no solo la captura): el grid bento (F3b), el match score real (F3a) y toda la capa de producto (biblioteca/social/RLS/APIs) funcionan correctamente en producción. Lo que nunca se tocó durante F1-F7 — porque F3a/F3b acotaron su alcance deliberadamente a `MediaCard`/`MediaGrid`, no al cromado alrededor — es la interfaz interactiva de Discover (`FilterBar`, pills de tipo, cabecera) y la navegación desktop (`NavLinks`), que siguen siendo UI utilitaria de antes de Bloque F (E59 y anteriores): funcional pero visualmente discreta, sin el peso Gen-Z del resto del rediseño. No es un fallo de arquitectura ni justifica reconstruir la aplicación — es continuar vistiendo con el mismo lenguaje visual ya aplicado a las cards.

- [x] **G1. Discover: barra de búsqueda + pills de tipo con más peso visual** — _cerrada 2026-09-12, commit `a70dd8c`_
  El mockup F0 muestra una barra de búsqueda prominente ("Busca por título, mood...") bajo la cabecera de Discover — hoy no existe ninguna en `/discover` (la búsqueda vive solo detrás del icono de lupa del header, page `/search` separada). Las pills de tipo (Película/Serie/Anime/...) en `DiscoverClient.tsx` son texto plano sin emoji ni distinción visual más allá del verde activo/inactivo. La cabecera "Descubrir" (`text-4xl`) es mucho más discreta que el tratamiento chunky del mockup.
  **Cierre:** (a) integrado `<SearchBar>` (ya existente, `/api/search` + debounce + navegación a `/search?q=`) bajo la cabecera con placeholder propio (`discover.searchPlaceholder`, +1 clave i18n es/en) — 0 lógica de búsqueda duplicada, mismo componente que ya usa `SearchClient`. (b) `TYPE_EMOJI` (mapa `DiscoverType→string`, ✨🎬📺⛩️📚🈷️🎮💬) renderizado en un `<span aria-hidden="true">` delante de cada label de pill — decorativo puro, el nombre accesible del radio (`getByRole('radio', {name:...})`) sigue siendo solo la etiqueta i18n, verificado con test nuevo; acento activo intacto (`accent-positive`, sin tocar F2). (c) `h1` de `text-4xl tracking-wide` → `text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight` (misma escala que el `h1` de `MediaDetail.tsx`, F4) + `IconCompass` propio (sistema F1b, no emoji/lucide) junto al título. +2 tests de regresión en `DiscoverClient.test.tsx` (búsqueda presente, emoji sin romper nombre accesible). Verificación visual real: harness temporal no commiteado (mismo patrón que F4) montando `DiscoverClient` con fixture de 8 items y `fetch('/api/discover')` interceptado, capturado con Chromium headless (mobile 390px + desktop 1440px) contra el CSS de Tailwind realmente compilado por `next dev` — confirma header/búsqueda/pills/grid/paginación sin regresión de layout. tsc 0, lint 0, vitest **1313 passed**.

- [x] **G2. Navegación desktop (`NavLinks`) con el mismo peso visual que la navegación móvil** — _cerrada 2026-09-12, commit `ed89e45`_
  `BottomNav` (móvil) ya usa el sistema de iconos propio (F1b) desde su creación; `NavLinks` (desktop, `src/components/layout/NavLinks.tsx`) seguía siendo enlaces de solo texto (`text-sm`) con un punto diminuto como único indicador de estado activo — visualmente muy por debajo del resto de Bloque F.
  **Cierre:** `NavLinks.tsx` pasa a icono+etiqueta (mismos 8 iconos sugeridos: `IconHome`/`IconCompass`/`IconLibrary`/`IconLists`/`IconChat`/`IconFriends`/`IconGroups`/`IconIdea`, sin desviarse de la lista propuesta — todos encajan bien con su item), con un tratamiento de pill activa (`bg-accent-positive text-on-accent-positive rounded-full`, mismo lenguaje que las pills de tipo de Discover) en vez del punto. Estado inactivo migrado a `text-text-secondary` (no `text-muted`, alias legacy), confirmado como el token ya usado en `MediaDetail.tsx` post-F5. Badge de no-leídos (E96) intacto en lógica/comportamiento/aria-label, solo reposicionado sobre el icono del chat (antes flotaba junto al texto); se le añadió `ring-2 ring-surface-default` para que no se funda visualmente con el fondo de la pill activa cuando `/chat` está seleccionado con no-leídos (mismo color de fondo que el header, invisible en el resto de casos).
  Verificación visual real: harness temporal (no commiteado, igual que en F4/F6) que monta el header real (`Logo`+`NavLinks` reales) con un `next.config.mjs` rewrite temporal (también revertido) para que `usePathname()` apunte de verdad a `/home` y `/library` — no una simulación, el mismo mecanismo de detección de ruta activa que corre en producción. Capturado con Chromium headless en desktop (1440px): pill activa cambiando de "Inicio" a "Mi biblioteca" entre las dos rutas, badge de chat con "7" y con "99+" (fetch de `/api/chat` parcheado solo dentro del harness), y en mobile (390px) confirmando que `hidden md:flex` sigue ocultando la nav sin romper el header (logo+campana).
  Test unitario actualizado (`tests/unit/components/layout/NavLinks.test.tsx`): añade cobertura de icono presente por link y de las clases de la pill activa/inactiva, sin perder ninguno de los casos ya cubiertos (8 links, badge ausente/"5"/"99+", href de chat). tsc 0, lint 0 (solo warnings preexistentes ajenos), vitest **1313 passed**.
