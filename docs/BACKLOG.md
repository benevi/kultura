...
- [x] **G1. Discover: barra de búsqueda + pills de tipo con más peso visual** — _cerrada 2026-09-12_
  ...
  Alcance: (a) añadir entrada de búsqueda a Discover, reutilizando `/api/search` existente (nunca duplicar lógica de búsqueda — regla técnica 2/3 de CLAUDE.md), (b) emoji por tipo en las pills de `TYPE_ORDER` (🎬📺⛩📚💬🈷🎮), manteniendo el verde (`accent-positive`) como color de estado activo — **no reabrir la decisión de paleta que F2 ya resolvió** con múltiples colores por tipo, eso confundiría la jerarquía de acento único, (c) subir la escala tipográfica de la cabecera "Descubrir" acorde al resto de Bloque F.
  Hecho cuando: verificación visual real (Chromium headless, mobile+desktop) mostrando los 3 cambios; tsc/lint/vitest en verde; ningún test existente de `DiscoverClient`/`FilterBar` roto por el cambio de estructura.
...
- [ ] **G2. Navegación desktop (`NavLinks`) con el mismo peso visual que la navegación móvil**
  `BottomNav` (móvil) ya usa el sistema de iconos propio (F1b) desde su creación; `NavLinks` (desktop, `src/components/layout/NavLinks.tsx`) sigue siendo enlaces de solo texto (`text-sm`) con un punto diminuto como único indicador de estado activo — visualmente muy por debajo del resto de Bloque F.
  Alcance: reusar los iconos ya existentes del sistema propio (`IconHome`, `IconCompass`, `IconLibrary`, `IconLists`, `IconChat`, `IconFriends`, `IconGroups`, `IconIdea` para sugerencias) con icono+etiqueta, y un tratamiento de pill activa (fondo `accent-positive`, mismo lenguaje que las pills de tipo de Discover) en vez del punto actual.
  Hecho cuando: verificación visual real (desktop, con y sin sesión de mensajes no leídos) del nuevo tratamiento; tsc/lint/vitest en verde; test de `NavLinks` actualizado sin perder cobertura (activo/inactivo, badge de no leídos, enlaces correctos).
