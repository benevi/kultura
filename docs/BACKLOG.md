...
- [ ] **E100. `components/ui/button.tsx`: foco de teclado usa tokens shadcn muertos (`ring-ring`, `ring-offset-background`)**
  Hallazgo de F5 (auditoría de accesibilidad post-rediseño): `focus-visible:ring-ring focus-visible:ring-offset-2` en `src/components/ui/button.tsx` no genera ninguna regla CSS real — `--ring`/`--background` no están registrados como colores en `tailwind.config.ts` (confirmado por grep en el CSS compilado: cero ocurrencias de `.focus-visible\:ring-ring`). No es un fallo de accesibilidad (Tailwind aplica su azul por defecto cuando el color no resuelve, así que el foco sigue siendo visible) pero es inconsistencia de marca — ya señalado como candidato en el cierre de F2 ("variantes con tokens shadcn muertos... candidato a nuevo hallazgo"), nunca registrado hasta ahora. Afecta a los consumidores reales de este componente: `RecommendButton`/`RecommendModal`, `AddToListButton`.
  Hecho cuando: el foco de teclado en esos botones usa el verde de marca (`accent-positive`, mismo patrón que `KButton`) en vez del azul por defecto de Tailwind, verificado en el CSS compilado real (no solo leyendo la clase).
