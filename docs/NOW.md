...
E79 slice 1 → ✅ **CERRADA 2026-06-16** (este commit). Paginación Descubrir "has-next": elimina páginas cortas/vacías por post-filtros. `DiscoverResult.hasMore` (aditivo) = `page < providerTotalPages` por familia (agregado `all` = `page*20 < merged.length`); `Pagination` gatea "next" por `!hasMore` (no por `totalPages`, inflado por los post-filtros temporadas/volumenes/game-suite/NSFW) y muestra "page X" sin "of Y". Elegido A1 sobre A2 (back-fill) porque el componente ya era prev/next → coste mínimo. tsc 0, lint 0, vitest **1157 passed**. **Slice 2 pendiente** (no en este commit): cachear pool agregado `all` (hoy re-fetcha page 1 ×7 por página) + comic (pide 100, tira ~80), y profundizar la paginación real más allá del pool ya traído. Detalle en DONE.md / BACKLOG E79.
